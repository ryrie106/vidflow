...
Spring을 사용한 Web Application에서 Security 설정을 하면 `Servlet Filter` 가 추가된다. `DispatcherServelet` 이 웹 요청을 받으면
...
Restful API Web 서비스에서는 form 기반 로그인을 할 수 없기 때문에

./src/main/java/io/github/ryrie/vidflow/security/JwtAuthenticationFilter.java
[souce,java]
include::../src/main/java/io/github/ryrie/vidflow/security/JwtAuthenticationFilter.java[lines=38..64]


./src/main/java/io/github/ryrie/vidflow/controller/UserController.java
[source,java]
include::../src/main/java/io/github/ryrie/vidflow/controller/UserController.java[lines=44..51]
